...
final class CustomerRefundAccountsReceivableService
{
    ...
    public function post(CustomerRefund $refund, AccountingPeriod $period, string $journalReference, User $actor): CustomerLedgerEntry
    {
        ...
        $amount = BigDecimal::of((string) $refund->total_amount)->toScale(self::MONEY_SCALE, RoundingMode::HALF_UP);
        $baseCredit = BigDecimal::of((string) $refund->base_credit_amount)->toScale(self::MONEY_SCALE, RoundingMode::HALF_UP);
        $effectiveRate = $baseCredit->dividedBy($amount, self::RATE_SCALE, RoundingMode::HALF_UP);
