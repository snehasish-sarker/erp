...
final class JournalEntryValidationService
{
    ...
    public function validateAndNormalize(
        ...
        $customers = Customer::query()
            ->whereIn('id', array_values(array_unique($customerIds)))
            ->get()
            ->keyBy('id');
        ...
        foreach ($lines as $index => $line) {
            ...
            if ($requireActiveAccounts && !$account->isActive()) {
                throw ValidationException::withMessages([
                    "lines.{$index}.account_id" => [
            ...
            if ($manualPosting && !$account->allowsManualPosting()) {
                throw ValidationException::withMessages([
                    "lines.{$index}.account_id" => [
            ...
            if ($lineBranchId !== (int) $branch->getKey()) {
                throw ValidationException::withMessages([
            ...
            if (
                $supplierId !== null
                && !$suppliers->get($supplierId) instanceof Supplier
            ) {
                throw ValidationException::withMessages([
                    "lines.{$index}.supplier_id" => [
            ...
            if (
                $customerId !== null
                && !$customers->get($customerId) instanceof Customer
            ) {
                throw ValidationException::withMessages([
                    "lines.{$index}.customer_id" => [
            ...
            if ($account->control_type === 'accounts_payable') {
                if ($supplierId === null || $customerId !== null) {
                    throw ValidationException::withMessages([
                        "lines.{$index}.supplier_id" => [
            ...
            if ($account->control_type === 'accounts_receivable') {
            ...
            $lineCurrencyCode = isset($line['currency_code'])
            ...
            if ($lineCurrencyCode !== $currencyCode) {
                throw ValidationException::withMessages([
                    "lines.{$index}.currency_code" => [
            ...
            $lineExchangeRate = isset($line['exchange_rate'])
                && trim((string) $line['exchange_rate']) !== ''
                    ? $this->positiveDecimal(
                        value: (string) $line['exchange_rate'],
                        scale: self::RATE_SCALE,
                        field: "lines.{$index}.exchange_rate",
                        label: 'exchange rate',
                    )
                    : $exchangeRateDecimal;

            if (!$lineExchangeRate->isEqualTo($exchangeRateDecimal)) {
                throw ValidationException::withMessages([
            ...
            $debit = $this->nonNegativeDecimal(
                ...
                field: "lines.{$index}.debit_amount",
                label: 'debit amount',
            );

            $credit = $this->nonNegativeDecimal(
                value: $line['credit_amount'] ?? '0',
                field: "lines.{$index}.credit_amount",
                label: 'credit amount',
            );
            ...
            if (
                ...
                throw ValidationException::withMessages([
                    "lines.{$index}.debit_amount" => [
                        'Each journal line must contain either a positive debit or a positive credit, but not both.',
                    ],
            ...
            $dueDate = $this->nullableDate(
                value: $line['due_date'] ?? null,
                field: "lines.{$index}.due_date",
            );
            ...
            $normalizedLines[] = [
                ...
                'exchange_rate' => (string) $lineExchangeRate,
                'debit_amount' => (string) $debit,
                'credit_amount' => (string) $credit,
                'base_debit_amount' => (string) $baseDebit,
                'base_credit_amount' => (string) $baseCredit,
            ];

            $totalDebit = $totalDebit->plus($debit);
            $totalCredit = $totalCredit->plus($credit);
            $baseTotalDebit = $baseTotalDebit->plus($baseDebit);
            $baseTotalCredit = $baseTotalCredit->plus($baseCredit);
        }
        ...
        if (!$baseTotalDebit->isEqualTo($baseTotalCredit)) {
            throw ValidationException::withMessages([
                'lines' => [
                    sprintf(
        ...
        return [
            'currency_code' => $currencyCode,
            'exchange_rate' => (string) $exchangeRateDecimal,
            'lines' => $normalizedLines,
            'total_debit' => (string) $totalDebit,
            'total_credit' => (string) $totalCredit,
            'base_total_debit' => (string) $baseTotalDebit,
            'base_total_credit' => (string) $baseTotalCredit,
        ];
    }
    ...
    public function normalizeExchangeRate(string $exchangeRate): string
    {
    ...
    private function decimal(
        ...
        try {
            return BigDecimal::of((string) $value)->toScale(
                $scale,
                RoundingMode::HALF_UP,
            );
