...
final class AccountsPayableAgingBucketRegistry
{
    /**
    ...
    public function label(string $bucket): string
    {
        ...
        if (!is_array($configuration)) {
            throw new LogicException(
                "Unsupported Accounts Payable aging bucket [{$bucket}].",
            );
        }
    ...
    public function bucketForDaysOverdue(
        int $daysOverdue,
    ): string {
        if ($daysOverdue <= 0) {
            return 'current';
        ...
        foreach (
            ...
            $options[] = [
                ...
                'label' =>
                    $configuration['label'],
                'minimum_days' =>
                    $configuration['minimum_days'],
                'maximum_days' =>
                    $configuration['maximum_days'],
            ];
        }
